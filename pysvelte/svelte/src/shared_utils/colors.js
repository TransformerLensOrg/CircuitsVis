...
function norm_nd(x, ord = 2) {
  ...
  for (let i = 0; i < x.shape[0]; i++) {
    S += Math.abs(x.get(i)) ** ord;
  }
  return S ** (1 / ord);
}

function normalize(v, ord = 2) {
  const v_norm = norm(v, ord);
...
export function hue_to_rgb(theta) {
  ...
  theta %= 360;
  const d_theta = 360 / colors.length;
  const n = Math.floor(theta / d_theta);
  const mix_coef = (theta - n * d_theta) / d_theta;
  let v = interp(colors[n], colors[(n + 1) % colors.length], mix_coef);
  v = normalize(v, 1);
  return v;
}
...
/*
 arguments:
  ...
  returns:
    RGB value as javascript list
 */
export function sparse_color_map(
  v,
  zero_c = [0.98, 0.98, 0.98],
...
/*
 arguments:
  ...
  returns:
    string representing color as a CSS value
 */
export function sparse_color_map_css(
  v,
  zero_c = [0.98, 0.98, 0.98],
  isolate_channel = undefined,
  hues = undefined
  ...
  const color = sparse_color_map(v, zero_c, isolate_channel, hues);
...
export function sparse_color_map_neuron_css(v, zero_c = [0.98, 0.98, 0.98]) {
  ...
  const color = sparse_color_map_neuron(v, zero_c);
  return rgb_to_css(color);
}
