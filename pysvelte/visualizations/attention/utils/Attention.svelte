<script>
    import { sparse_color_map_css } from "./colors";
    import { default as ArrayImage } from "./ArrayImage.svelte";
    import LockableValueToggle from "./LockableValueToggle.svelte";
    ...
    function reduce_Y(arr){
        ...
        for (var x=0; x < arr.length; x++){
            arr_.push([])
            for (var c=0; c < arr[0][0].length; c++){
                var temp = 0;
                for (var y=0; y < arr[0].length; y++){
                    temp = Math.max(temp, arr[x][y][c]);
                }
                arr_[x].push(temp);
            }
        }
...
<div class="attn-container">
    ...
    <!-- {#if N_heads > 1}
        ...
        <div class="heads">
            {#each range(attention_show[0][0].length) as head_i}
                <LockableValueToggle
