...
def log_prob_vis(
    ...
    """Takes the log probs for a model on some text. 

    Outputs the tokens coloured by the log prob, and on hover shows you the top
    K tokens that the model guessed for that position, and where the true token
    ranked in that.
    ...
    assert len(log_probs.shape) == 2, \
        f"Log Probs shape must be 2D: {log_probs.shape}"
    assert len(token_indices.shape) == 1, \
        f"Tokens shape must be 1D: {token_indices.shape}"
    assert token_indices.size(0) == log_probs.size(0), \
        "Number of tokens and log prob vectors must be identical, " + \
        f"{log_probs.shape}, {token_indices.shape}"
    ...
    _values, sorted_log_prob_indices = log_probs.sort(
    ...
    assert len(correct_token_rank) == (len(token_indices) - 1), \
        "Some token indices were missing from sorted_log_prob_indices"
